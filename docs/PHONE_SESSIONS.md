...
1. **Bot**: clone the repo, run `install.bat`, then `start.bat`
   (bot starts in DRY RUN as always).
...
4. **Phone view**: the Streamlit operator dashboard has been **RETIRED**.
   The phone view is now **Grafana Cloud** (see below), which needs no
   tailnet and no open port on the PC; control is the git remote-control
   plane (`scripts/remote_control.py`). Tailscale is only needed for the
   optional moomoo-over-tailnet feed (below): install it on the PC and
   sign in (same account as the phone).
...
one learning stream (`paper-telemetry`), with the phone watching the bot
through Grafana Cloud (below).
...
- **Dashboard source of truth**: `docs/grafana/liquiditybot_command.json`
  (uid `liquiditybot-trading`) — one condensed logistical board generated by
  `scripts/build_trading_dashboard.py` (never hand-edit). It replaces the old
  monolith + the four graph partitions + the incidents board. Import/update
  via the HTTP API (`POST /api/dashboards/db`, service-account token,
  `scripts/grafana_import.py`) or the UI.
