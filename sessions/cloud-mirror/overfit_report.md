# Overfit audit — 2026-07-31 03:44 UTC

Dataset: live history (2022 rows)

- **FAIL** gap[logistic]: OOF gap within memorization band — train_auc=0.744 oof_auc=0.510 gap=+0.233
- **FAIL** gap[gbt]: OOF gap within memorization band — train_auc=0.689 oof_auc=0.505 gap=+0.184
- **FAIL** gap[mlp]: OOF gap within memorization band — train_auc=0.719 oof_auc=0.521 gap=+0.198
- **PASS** shuffle: destroyed labels learn nothing OOF — mean_auc=0.506 z=0.8 (limit 3.0)
- **INFO** pbo space — ml.adaptive_gbt.enabled=true — the adaptive rung is IN the measured selection space
- **INFO** pbo — evidence-gated to a single family (no model selection to overfit at this live-row count) (space=['logistic'])
- **PASS** purge: never manufactures out-of-sample edge — unpurged=0.486 purged=0.492 leak_closed=-0.006
- **INFO** purge note — expanding-window design keeps boundary leak ~0 by construction; shuffle-null [OF-2] is the leak gate
- **PASS** dof: not starved (>=10 rows per feature) — rows/feature=31.6 (2022 rows / 64 features)
- **FAIL** dof: dead-feature fraction under 55% (live data) — dead_frac=0.70 (45 near-zero-importance features)
- **INFO** dof note — low/zero-importance: ['hour_sin', 'pd_zone', 'gate_confidence', 'funding_dist', 'other_ret_6_dir', 'flow_tox'] ...
...
- **INFO** regime[range] oof — oof_n=880 auc=0.475 (pooled 0.505, delta_auc=-0.030) brier=0.2458 (pooled 0.2493, delta_brier=-0.0034)
...
- **INFO** regime[bear] oof — oof_n=805 auc=0.499 (pooled 0.505, delta_auc=-0.006) brier=0.2531 (pooled 0.2493, delta_brier=+0.0038)
- **INFO** regime[crisis] — n=0 — absent from corpus
- **INFO** regime[unknown] — n=0 — absent from corpus
- **INFO** learning curve caveat — each point refits gbt on a chronological PREFIX of the corpus (time-purged OOF, deployed protocol) — points are the same statistic across sizes, but none is OF-1's own pooled number
- **INFO** lc[n=506] — oof_n=420 auc=0.444 brier=0.2498
- **INFO** lc[n=809] — oof_n=670 auc=0.521 brier=0.2416
- **INFO** lc[n=1112] — oof_n=925 auc=0.460 brier=0.2572
- **INFO** lc[n=1415] — oof_n=1175 auc=0.481 brier=0.2402
- **INFO** lc[n=1719] — oof_n=1430 auc=0.476 brier=0.2441
- **INFO** lc[n=2022] — oof_n=1685 auc=0.478 brier=0.2505
