...
- **FAIL** gap[gbt]: OOF gap within memorization band — train_auc=0.754 oof_auc=0.527 gap=+0.226
- **FAIL** gap[mlp]: OOF gap within memorization band — train_auc=0.745 oof_auc=0.505 gap=+0.241
...
- **INFO** null-floor[gbt] — OOF Brier 0.2382 vs base-rate constant 0.1861 (base=0.247, n=1780) — LOSES TO THE NULL: negative skill, sizing on this model's p(win) is worse than sizing on the base rate
- **INFO** null-floor[mlp] — OOF Brier 0.2798 vs base-rate constant 0.1861 (base=0.247, n=1780) — LOSES TO THE NULL: negative skill, sizing on this model's p(win) is worse than sizing on the base rate
- **PASS** shuffle: destroyed labels learn nothing OOF — mean_auc=0.491 z=1.1 (limit 3.0)
...
- **FAIL** dof: dead-feature fraction under 55% (live data) — dead_frac=0.72 (46 near-zero-importance features)
- **INFO** dof note — low/zero-importance: ['funding_dir', 'ret_6_dir', 'depth_ratio', 'fvg_pull', 'poc_dist', 'ret_12_dir'] ...
...
- **INFO** lc[n=1820] — oof_n=1515 auc=0.469 brier=0.2501
- **INFO** lc[n=2141] — oof_n=1780 auc=0.500 brier=0.2617
- **INFO** learning curve trend FLAG — DECLINING (delta_auc=-0.034 < -0.03) — later rows are HURTING skill: regime/era drift inside the training window (check ML-080 mix drift and era_exclusion)
