...
- Activity: 2 open | 256 live labeled trades | 6826 candidates | 17 postmortems
...
- [WARN] **SD-002 model starvation loop**  -  model is cold (live training rows=256, brier=n/a) yet retrain was requested 121x  -  with 0 entries there is no new data, so retraining can never clear the condition. Seed a model (scripts/train_meta.py) or supply history; this loop is also 22% of the audit trail
