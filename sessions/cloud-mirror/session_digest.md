...
- Activity: 2 open | 297 live labeled trades | 9191 candidates | 17 postmortems
- Model: level 0 | use_model=True | brier n/a | history_rows 297 | cold=True
...
- [WARN] **SD-002 model starvation loop**  -  model is cold (live training rows=297, brier=n/a) yet retrain was requested 121x  -  with 0 entries there is no new data, so retraining can never clear the condition. Seed a model (scripts/train_meta.py) or supply history; this loop is also 22% of the audit trail
