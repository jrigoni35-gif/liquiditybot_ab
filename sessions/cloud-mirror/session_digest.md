...
- Activity: 0 open | 241 live labeled trades | 4625 candidates | 17 postmortems
- Model: level 1 | use_model=True | brier n/a | history_rows 241 | cold=True
...
- [WARN] **SD-002 model starvation loop**  -  model is cold (live training rows=241, brier=n/a) yet retrain was requested 120x  -  with 0 entries there is no new data, so retraining can never clear the condition. Seed a model (scripts/train_meta.py) or supply history; this loop is also 22% of the audit trail
