...
_LEARNING_PANELS = frozenset({
    ...
    # overfit floor gates LOADED rows — the gauge counts RAW rows and must
    # say so. Its LOADED companion (id 15, added later the same day when
    # liquiditybot_ml_loaded_rows landed) renumbers everything below.
    (14, "Raw training rows collected", "gauge",
     ("liquiditybot_ml_history_rows",)),
    (15, "Rows the trainer actually used (as of last retrain)", "gauge",
     ("liquiditybot_ml_loaded_rows",)),
    (16, "New-era rows toward re-arm", "gauge",
     ("liquiditybot_era_excl_new_rows",)),
    (17, "Labels per day", "stat",
     ("liquiditybot_probe_budget_live_labels_per_day_7d",)),
    (18, "Labels in the last 24h", "stat",
     ("liquiditybot_probe_budget_labels_24h",)),
    (19, "Label uniqueness", "stat", ("liquiditybot_ml_mean_uniqueness",)),
    (20, "Corpus growth", "timeseries",
     ("liquiditybot_ml_history_rows", "liquiditybot_ml_live_clean")),
    (21, "Where labels come from", "timeseries", ("liquiditybot_ml_labels",)),
    (22, "What the labels say", "row", ()),
    (23, "How this era's trades ended", "piechart",
     ("liquiditybot_era_reason_rows",)),
    (24, "Label rate this era", "stat", ("liquiditybot_era_label_rate",)),
    (25, "Corpus drift distance", "stat", ("liquiditybot_era_mix_tvd",)),
    (26, "Corpus matches live?", "stat", ("liquiditybot_era_mix_alarm",)),
    (27, "Rows excluded from training", "stat",
     ("liquiditybot_era_excl_dropped",)),
    (28, "Which model is driving", "row", ()),
    (29, "Deployed model over time", "timeseries",
     ("liquiditybot_ml_model_info",)),
    (30, "Retrain queued", "stat", ("liquiditybot_ml_retrain_flag",)),
    (31, "Retrain failures", "stat", ("liquiditybot_ml_retrain_failures",)),
    (32, "Model fallbacks", "stat", ("liquiditybot_ml_model_fallbacks",)),
    # ledger-derived aux panels (2026-08-17): the first board consumers of
    # gc_pusher.collect_aux() — registry lifecycle counts and the ML-083
    # orphan ratio (a scale-free MULTIPLE, unit x, never percentunit)
    (33, "Model lifecycle events", "bargauge",
     ("liquiditybot_ml_lineage_events",)),
    (34, "Is the model orphaned?", "stat",
     ("liquiditybot_ml_orphan_ratio",)),
    # the era-4 verdict clock: COUNT and pre-registered target ONLY — the
    # accruing gate means exist on no board by law (accrual moratorium)
    (35, "The verdict clock", "row", ()),
    (36, "Era-4 verdict progress", "bargauge",
     ("liquiditybot_cohort_closes", "liquiditybot_cohort_min_n")),
    (37, "The cost of learning", "row", ()),
    (38, "Probe tokens in the tank", "gauge",
     ("liquiditybot_probe_budget_tokens",)),
    (39, "Tuition spent in 24h", "stat",
    ...
    (42, "Probes open now", "stat",
     ("liquiditybot_probe_budget_open_probes",)),
    (43, "Denied - budget empty", "stat",
     ("liquiditybot_probe_budget_denied_exhausted_24h",)),
    (44, "Probe governor", "stat",
     ("liquiditybot_probe_budget_governor_factor",)),
    (45, "Refunds in 24h", "stat",
     ("liquiditybot_probe_budget_refunds_24h",)),
    (46, "Gate learning", "row", ()),
    (47, "Learned gate weights", "bargauge", ("liquiditybot_gate_weight",)),
    (48, "Is any gate lying?", "bargauge",
     ("liquiditybot_gate_divergence",)),
    (49, "Labeled rows feeding the gates", "stat",
     ("liquiditybot_gate_labeled",)),
    (50, "Base win rate the gates see", "stat",
