...
- **Era-confound class**: never compare or pool statistics across disjoint `label_era` populations; `gate_efficacy_report.py` refuses with CONFOUNDED_BASELINE/PARTIAL_OVERLAP since `62ab10c0`. `label_era_of()` without horizon knowledge is the documented era-mixing defect — persisted era first, unknown excluded from overlap credit.
